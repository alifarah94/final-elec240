...
void WaitLcdBusy(void)
{
	...
	short int IDRvalue = 1; // set IDR to 1
	set_LCD_bus_input();    // making pin input
	clr_LCD_RS();           // Setting RS low
	set_LCD_RW();	          // Setting RW high
	
	// A while loop that checks the last bit to see if busy.
	while(IDRvalue != 0){
	    set_LCD_E();
		  lcd_delayus(1);
	 	  IDRvalue = GPIOD->IDR;
